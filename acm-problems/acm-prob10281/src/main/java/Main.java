...
public class Main {
  ...
  private static void process(List<CarSpeedData> dataList) {
	...
	for (int i = 1, n = dataList.size(); i < n; i++) {
	  ...
	  if (!currentData.isSpeedGiven()) {
		BigDecimal d = currentData.getSpeed().multiply(timeDelta).divide(SECONDS_PER_HOUR_BIGDECIMAL, 2, BigDecimal.ROUND_HALF_UP);
		totalDistance = totalDistance.add(d);
		System.out.printf("%s %s km%n", currentData.getFormattedTime(), totalDistance.toString());
	  } else {
		BigDecimal d = previousData.getSpeed().multiply(timeDelta).divide(SECONDS_PER_HOUR_BIGDECIMAL, 2, BigDecimal.ROUND_HALF_UP);
		totalDistance = totalDistance.add(d);
	  }
	}
  }
